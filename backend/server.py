...
# ----------------------- Default Portfolio Content (seed) -----------------------
DEFAULT_CONTENT: Dict[str, Any] = {
    "site": {
        "name": "Kyllian Le Guen",
        "logo": "KLG.",
    ...
    "hero": {
        ...
        "stats": [
            ...
            {"value": "BUT", "label": "Génie Mécanique"},
        ],
    },
    "marquee": [
        "3DExperience", "Smart 3D", "Catia V5", "SolidWorks", "PowerMill",
        "Fabrication Additive", "VBA · Python", "Réalité Virtuelle",
        "Unity 3D", "NCSimul", "Industrie 4.0",
    ],
    "projects": [
        {
            "id": "p1",
            "num": "01",
            "title": "Usinage 5 axes — Pièce complexe inspirée du cubisme",
            "category": "Usinage / FAO",
            "subtitle": "Génie mécanique & fabrication",
            "description": "Programmation complète d'un parcours FAO pour l'usinage d'une pièce complexe à formes gauches, de l'ébauche à la finition, avec optimisation de l'état de surface sur centre 5 axes.",
    ...
    "education": [
        {
            ...
            "tag": "Formation",
            "details": [
                "Conception mécanique : logiciels CAO (3DExperience, Catia) pour modéliser et simuler des systèmes mécaniques",
    ...
    "experiences": [
        {
            ...
            "missions": [
                ...
                "Standardisation et amélioration continue des processus d'intégration réseaux",
                "Formation au logiciel Smart 3D — CAO industrielle en environnement naval",
        ...
        {
            "id": "x2",
            "company": "Halle 6 Ouest",
            "role": "Intégrateur de cellule d'impression 3D robotisée",
            "type": "Stage",
    ...
    "passions": {
        "sports": [
            {
                ...
                "tag": "Pratique régulière · Objectif amélioration continue",
        ...
        "travels": {
            "description": "Les voyages ont forgé mon ouverture d'esprit et ma capacité d'adaptation — des qualités que j'applique au quotidien dans mes collaborations professionnelles internationales.",
            "countries": ["Norvège", "Espagne", "Malte", "Angleterre"],
        },
    },
    "contact": {
...
# Simple in-memory token store (single admin, single password)
...
@api_router.get("/portfolio/content")
async def get_content():
    doc = await db.config.find_one({"key": "content"}, {"_id": 0})
    if not doc:
        await db.config.insert_one({"key": "content", "value": DEFAULT_CONTENT})
        return DEFAULT_CONTENT
    return doc["value"]


@api_router.put("/portfolio/content")
async def update_content(content: Dict[str, Any], _: bool = Depends(require_admin)):
    await db.config.update_one(
        {"key": "content"},
        {"$set": {"key": "content", "value": content}},
        upsert=True,
    )
    return {"status": "ok"}


@api_router.post("/portfolio/content/reset")
async def reset_content(_: bool = Depends(require_admin)):
    await db.config.update_one(
        {"key": "content"},
        {"$set": {"key": "content", "value": DEFAULT_CONTENT}},
        upsert=True,
    )
    return {"status": "ok", "value": DEFAULT_CONTENT}


@api_router.get("/portfolio/settings")
async def get_settings():
    doc = await db.config.find_one({"key": "settings"}, {"_id": 0})
    if not doc:
        await db.config.insert_one({"key": "settings", "value": DEFAULT_SETTINGS})
        return DEFAULT_SETTINGS
    return doc["value"]


@api_router.put("/portfolio/settings")
async def update_settings(settings: SettingsInput, _: bool = Depends(require_admin)):
    await db.config.update_one(
        {"key": "settings"},
        {"$set": {"key": "settings", "value": settings.model_dump()}},
        upsert=True,
    )
    return {"status": "ok"}


@api_router.post("/portfolio/login")
...
@api_router.post("/portfolio/logout")
...
@api_router.get("/portfolio/verify")
...
async def list_messages(_: bool = Depends(require_admin)):
    items = await db.messages.find({}, {"_id": 0}).sort("createdAt", -1).to_list(1000)
    return items


@api_router.delete("/admin/messages/{msg_id}")
async def delete_message(msg_id: str, _: bool = Depends(require_admin)):
    await db.messages.delete_one({"id": msg_id})
    return {"status": "ok"}


@api_router.patch("/admin/messages/{msg_id}/read")
async def mark_read(msg_id: str, _: bool = Depends(require_admin)):
